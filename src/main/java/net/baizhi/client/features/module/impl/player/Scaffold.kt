...
import net.baizhi.client.utils.block.BlockUtils.canBeClicked
import net.baizhi.client.utils.block.BlockUtils.isReplaceable
import net.baizhi.client.utils.block.PlaceInfo
...
class Scaffold : Module() {
    ...
    private fun search(blockPosition: BlockPos): Boolean {
        ...
        if (rotationModeValue.get().equals("GrimAC", ignoreCase = true)) {
            val (gcdYaw, gcdPitch) = GrimACUtil.applyDynamicRotation(
            ...
            val grimRot = Rotation(gcdYaw, gcdPitch)
            RotationUtils.setTargetRotation(grimRot, 0)
            lockRotation = grimRot
            faceBlock = true
        } else {
